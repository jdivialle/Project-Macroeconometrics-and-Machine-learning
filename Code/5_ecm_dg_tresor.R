...
base_opale <- select(cntbis, DATE, smpt_cnt_marng, prod_travail_marng, prod_tete_marng) |>
  arrange(DATE) |>
  filter(DATE <=as.Date('2022-06-01')) |>
  mutate(log_smpt = log(smpt_cnt_marng)) |>
  mutate(log_prod_tete = log(prod_tete_marng)) |>
  mutate(log_prod_travail = log(prod_travail_marng)) |>
...
ipc_cvs <- get_insee_idbank('001769682') |>
  select(DATE, OBS_VALUE) |>
  rename(ipc_cvs = OBS_VALUE) |>
  arrange(DATE) |>
  mutate(DATE = as.Date(DATE)) |>
  filter(DATE <=as.Date('2022-06-01') & substr(DATE, 6, 7) == "03" |substr(DATE, 6, 7) == "06"|substr(DATE, 6, 7) == "09"|substr(DATE, 6, 7) == "12") |>
  mutate(ipc_cvs = ipc_cvs/ipc_cvs[DATE=='2019-12-01']*100) |>
  mutate(log_ipc_cvs = log(ipc_cvs))

base_opale <- left_join(base_opale, ipc_cvs, by = "DATE")
base_opale <- left_join(base_opale, chomage_BIT, by = "DATE")
base_opale <- base_opale |>
  relocate(smpt_cnt_marng, log_smpt, ipc_cvs, log_ipc_cvs, chomage_BIT, .after = prod_tete_marng)
...
base_opale <- base_opale |>
  ...
  mutate(ind_t1 = ifelse(substr(DATE, 6, 7) == "03", 1, 0)) |>
...
base_opale_2<- select(base_opale, DATE, log_smpt, log_ipc_cvs, lag_log_smpt , lag_log_ipc_cvs, chomage_BIT, log_prod_tete, moy_mobile_prod_tete, ind_2008t4_2010t4, ind_t1, y_long_term)

###On fait des tests de racines unitaires (séries dans la relation de LT intégrées d'ordre 1)
y_lt_ts <- ts(base_opale_2$y_long_term, start=c(1995,1), end=c(2018, 4), frequency=4)
ind_ts <- ts(base_opale_2$ind_2008t4_2010t4, start=c(1995,1), end=c(2018, 4), frequency=4)
smpt_ts <- ts(base_opale_2$log_smpt, start=c(1995,1), end=c(2018, 4), frequency=4)
ipc_ts <- ts(base_opale_2$log_ipc_cvs, start=c(1995,1), end=c(2018, 4), frequency=4)
prod_ts <- ts(base_opale$moy_mobile_prod_tete, start=c(1995,1), end=c(2018, 4), frequency=4)
...
lt_lm_relation <- lm(log_smpt~lag_log_ipc_cvs + moy_mobile_prod_travail+ind_2008t4_2010t4, data = base_opale)
lt_lm_relation <- lm(y_long_term~ind_2008t4_2010t4, data = base_opale_2)
...
error_lt_relation <- arrange(error_lt_relation, time) |>
  ...
  filter(time !=1)
base_opale_2$time <- seq(1,96) 
base_opale_2 <- base_opale_2 |>
  arrange(DATE) |>
  ...
  mutate(dif_chom = chomage_BIT/100 - lag(chomage_BIT, 1)/100)

base_estim_ecm <- left_join(error_lt_relation, base_opale_2, by ="time")
...
ecm_estimation <- lm(dif_smpt~dif_ipc+dif_lag_ipc+dif_chom + dif_prod+ ind_2008t4_2010t4+error_lag_lt, data = base_estim_ecm)
...
base_time1 <- select(base_opale_2, time, log_smpt) |>
  filter(time ==1) |>
  mutate(time = 2) |>
...
###graphiques
#En niveau 
level_ecm_fr <- ggplot(aes(x = DATE), data = base_estim_ecm) + geom_line(aes(y = predict_level_smpt, color = "Prediction")) +
...
gliss_ecm_fr <- ggplot(aes(x = DATE), data = base_estim_ecm) + geom_line(aes(y = gliss_year_pred_smpt, color = "Prediction")) +
...
# library(haven)
# write_dta(
#   base_opale_estimation,
#   file.path("ecm_ofce.dta"),
#   version = 14,
#   label = attr(data, "label"))
# 
# base_opale_prediction <- base_opale_2[base_opale_2$DATE>'2012-12-01' & base_opale_2$DATE<='2019-12-01',]
# xtr <- base_opale_estimation[c('log_ipc_cvs', 'lag_log_ipc_cvs', 'log_prod_tete')]
# xeq <- base_opale_estimation[c('y_long_term', 'moy_mobile_prod_tete', 'ind_2008t4_2010t4')]
# 
# model1 <- ecm(base_opale_estimation$log_smpt, xeq, xtr,  lags = 1, weights = NULL, includeIntercept=TRUE)
# tab_model(model1)
# summary(model1)
# 
# base_opale_prediction$model1Pred <- ecmpredict(model1, base_opale_prediction, base_opale_prediction$log_smpt[1])
# base_opale_prediction <- mutate(base_opale_prediction, pred_smpt = exp(model1Pred)) |>
...
# 
# 
# ggplot(aes(x = DATE), data = base_opale_prediction) + geom_line(aes(y = real_smpt), color = "red") + 
#   geom_point(aes(y = real_smpt), color = "red") +
#   geom_line(aes(y = pred_smpt), color = "blue") + 
#   geom_point(aes(y = pred_smpt), color = "blue") + theme_bw() 
# 
# ggplot(aes(x = DATE), data = base_opale_prediction) + geom_line(aes(y = gliss_annuel_real_smpt), color = "red") + 
